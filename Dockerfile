...
# 6. Zmieniamy domyślną komendę (CMD). Rejestruje skaner, a potem odpala 
#    oryginalny skrypt startowy twórców scanservjs.
CMD brsaneconfig4 -r "$BR_NAME" 2>/dev/null; \
    brsaneconfig4 -a name="$BR_NAME" model="$BR_MODEL" ip="$BR_IP" && \
    echo "Skaner Brother skonfigurowany pomyślnie!" && \
